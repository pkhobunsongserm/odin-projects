...
function Education() {
    ...
    const addFormFields = (event) => {
        event.preventDefault();
        setFormData((prevFormData) => [...prevFormData, {school: "", degree: "", eduStarted: new Date(), eduCompleted: new Date()}])
    }
          ...
          <form id="educationFormContainer">
            {formData.map((element, index) => (
                <div className='form-fields' key={index}>
                    ...
                    <input type="text" id="school" name="school" value={element.school || ""} onChange={(event) => handleChange(index,event)}/>

                    <label htmlFor="degree">Degree Name: </label>
                    <input type="text" id="degree" name="degree" value={element.degree || ""} onChange={(event) => handleChange(index,event)}/>

                    <label htmlFor="eduStarted">Date Started: </label>
                    <DatePicker showIcon id="eduStarted" selected={element.eduStarted} onChange={(date) => handleChangeStarted(index,date)} />

                    <label htmlFor="eduCompleted">Date Completed: </label>
                    <DatePicker showIcon id="eduCompleted" selected={element.eduCompleted} onChange={(date) => handleChangeCompleted(index,date)} />
